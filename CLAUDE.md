...
依頼された変更、または明確に必要な変更のみを行ってください。

#### 原則

- **修正前に「最小の変更は何か」を自問する**
- **設定の問題はまず設定で解決する**（コード変更は最後の手段）
- **依頼されていないリファクタリングを混ぜない**
- **修正案を実行する前にユーザーに提示して確認を取る**
- **既存の同等機能がある場合、その実装に合わせて統一する**（独自の仕様を追加しない）

#### 例

**推奨 ✅**
- `.env` の値が間違っている → `.env` を修正する
- バグ修正 → バグの原因箇所だけ修正する

**避けるべき ❌**
- `.env` の問題なのにコードのリファクタリングも一緒に行う
- バグ修正のついでに周辺コードを「改善」する
...
**最終更新**: 2026-03-02
**更新内容**: 最小限の変更ルールを追加、コミットメッセージNG例を修正、MCP禁止ルールを緩和
