...
本プロジェクトは **PostgreSQL 16 + pgvector** の単一データベース構成で運用する。

| DB | 用途 | 接続名 |
|---|---|---|
| **PostgreSQL 16 + pgvector** | アプリ本体・チャットボット共通 | `pgsql` (default) |

pgvector 拡張は `faq_chunks` テーブルでのみ利用する。他テーブルは通常の PostgreSQL 型のみを使用する。

> 履歴: v1 では MySQL + pgsql_chatbot の 2 接続構成だったが、Cross-DB 参照（`chat_messages.user_id → users.id`）で FK 制約が張れず運用コストも倍増するため、PostgreSQL 単一構成に統合した（詳細は要件定義 v5.1）。
...
> DDL 記法について: 以下の DDL は概念表現である。`ENUM(...)` は Laravel Schema Builder が PostgreSQL の `VARCHAR + CHECK 制約` に自動変換する。`BIGINT UNSIGNED AUTO_INCREMENT` は PostgreSQL では `BIGINT GENERATED BY DEFAULT AS IDENTITY` に相当する。実際の DDL はマイグレーションファイルを参照。
...
### FAQチャンクのリレーション
- `faq_chunks` はアプリ本体テーブルとの FK を張らない（RAG 再インデックスで頻繁に DELETE/INSERT が走るため疎結合を維持）
- `chat_messages.context_chunks` (JSON) にチャンクIDを記録することで参照を追跡
...
### faq_chunks（同一 DB・pgvector 利用、参考）
...
### 文字コード
- PostgreSQL のデフォルト `UTF8` エンコーディングを使用（絵文字対応）
- 照合順序は `en_US.UTF-8` / `C.UTF-8` など環境デフォルト。ソート順を区別する検索は未使用
