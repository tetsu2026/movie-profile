...
graph TD
    ...
    E -.->|全画面右下| CB[チャットボットウィジェット<br/>全認証後ページ共通]
...
### チャットボットウィジェット（全認証後ページ共通・非ページ）
**認証**: 必要（一般ユーザー、管理者）
**説明**: ログイン後の全画面で右下に表示されるフローティングチャットUI。独立したページではなく、`layouts/app.blade.php` に組み込まれる共通ウィジェット。
...
**連携エンドポイント（内部API）**:
- `POST /chatbot/message`: 質問送信・応答受信
- `GET /chatbot/history`: 会話履歴取得（初回展開時）

**遷移先**:
- なし（ウィジェット内で完結）

**備考**:
- `auth` middleware必須（未認証ページでは表示しない）
- レート制限: 10リクエスト/分 per ユーザー
- 入力制限: 2000文字以内
- 履歴の表示は現セッションのユーザー分のみ
- Alpine.jsで状態管理（`x-data`, `x-show`, `@click`等）
...
- 一般ユーザー: ダッシュボード、プロフィール編集、動画管理、プレビュー、チャットボットウィジェット
