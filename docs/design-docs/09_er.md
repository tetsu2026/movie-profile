...
### faq_chunks ER図（同一 DB、pgvector 利用）
...
- **主要属性**:
  ...
  - `content`: メッセージ本文
  - `context_chunks`: ボット応答時に参照したFAQチャンクIDの配列(JSON)。ヒットなし（フォールバック応答）の場合は `NULL`（フォールバック判別キーとして兼用）
- **備考**:
  - 追記のみ（`updated_at` / `deleted_at` を持たない）
  - ユーザー削除時はカスケード削除
  - 履歴取得時は `context_chunks IS NULL` のアシスタント行（フォールバック応答）とその直前のユーザー質問をペアで除外
...
- **主キー**: `id` (BIGSERIAL)
- **備考**:
  - アプリ本体と同一 PostgreSQL DB 内のテーブル。`faq_chunks` でのみ pgvector 拡張（`CREATE EXTENSION vector`）を利用
  - `VECTOR(1024)` 型（Titan Embeddings v2 の出力次元）
  - HNSWインデックス（`vector_cosine_ops`）で近似最近傍検索を高速化、類似度 0.3 以上の top-3 を取得
  - `updated_at` カラムはファイルの mtime と比較して差分インデックスを判定（`php artisan chatbot:index` 引数なし時）
  - アプリ本体テーブルと FK 関係を持たない（RAG 再インデックスで頻繁に DELETE/INSERT が走るため疎結合を維持、`chat_messages.context_chunks` で参照追跡）
