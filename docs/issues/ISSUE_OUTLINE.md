...
**概要**: 公開プロフィールページおよびプレビューページで、実際に動画を再生表示する機能を実装。サムネイル動画（自動再生・ループ・ミュート）とポップアップ動画（モーダル表示・音声あり）を実装。
...
**受け入れ基準(AC)**:
- [ ] サムネイル動画が円形で画面右下に固定表示され、自動再生・ループ・ミュートで再生される
- [ ] サムネイルクリックでポップアップ動画がモーダル表示される
- [ ] ポップアップ動画は音声あり、コントロール（再生/一時停止、音量、シークバー）が表示される
